...
from models.smegcn import SMEGCN
from models.util import motif_process
...
class ST_SME_Conv(nn.Module):
    ...
    def __init__(
        ...
        super(ST_SME_Conv, self).__init__()
        ...
        self._graph_conv = SMEGCN(args,
                                  in_channels=hidden_channels,
                                  out_channels=hidden_channels, )
        self._graph_conv_test = SMEGCN(args,
                                  in_channels=8,
                                  out_channels=8, )
    ...
    def forward(
        ...
        # edge_index: torch.Size([2, 547])
        edge_index, edge_weight = motif_process(edge_index, self.args.num_nodes, path=self.args.file_path)
        ...
        # edge_index: torch.Size([2, 1450])
        # edge_weight: torch.Size([4, 1450])
        # T_0: torch.Size([64, 18, 358, 16])
        ...
        # test =  torch.randn(64,32,561,8).cuda()
        # T_test = torch.zeros_like(test).to(test.device)
        # edge_index_test = torch.zeros(2,9001)
        # edge_weight_test = torch.zeros(7,9001)
        # print('T_test:',T_test.shape)
        # for b in range(test.size(0)):
        #     for t in range(test.size(1)):
        #         T_test[b][t] = self._graph_conv_test(test[b][t], edge_index_test, edge_weight_test)        
        # print('T_test:',T_test.shape)
